...
#トルクの設定
print("ラズベリーパイから出力する電圧を打ち込んでください")
V = float(input())
tau1_base = 0.3112 * V #定数は(減速比*定格トルク)/5
tau2_base = 0.3112 * V

# 外力方向の設定
speed = 1  #外力方向を示すベクトルの大きさ（描画にのみ関係する）
direction_vector = [speed * math.cos(direction), speed * math.sin(direction)]
arrow_scale = 0.25 #外力方向を表示する大きさを決める変数
...
if update_flag:
    ...
    xe1 = xe1 * length_scale
    xe2 = xe2 * length_scale
    ...
    #外力方向
    ax.quiver(xe1, xe2, arrow_dx1, arrow_dx2, 
            angles='xy', scale_units='xy', scale=1, color='red',label='direction movement')

    # τ1 Force
    ax.quiver(xe1, xe2, f_tau1[0] * scale, f_tau1[1] * scale, 
            angles='xy', scale_units='xy', scale=1, color='green', label=f"τ1 Force : {f_tau1}[N]")

    # τ2 Force
    ax.quiver(xe1, xe2, f_tau2[0] * scale, f_tau2[1] * scale, 
            angles='xy', scale_units='xy', scale=1, color='orange', label=f"τ2 Force : {f_tau2}[N]")

    # Reverse Force
    reverse_vector = np.round(reverse_vector, 4)
    if reverse_flag:
        ax.quiver(xe1, xe2, reverse_vector[0] * scale, reverse_vector[1] * scale, 
                angles='xy', scale_units='xy', scale=1, color='purple', label=f"reverse_force : {reverse_vector}[N]")
    else:
        ax.quiver(xe1, xe2, reverse_dx1 , reverse_dx2, 
                angles='xy', scale_units='xy', scale=1, color=(0.5, 0.4, 0.7), label="cannot present") 

    #力の提示範囲の描画
    polygon_points = np.array([[xe1,xe2], 
                               [xe1 + f_tau1[0]*scale , xe2 + f_tau1[1]*scale], 
                               [xe1 + F_total[0]*scale, xe2 + F_total[1]*scale],
                               [xe1 + f_tau2[0]*scale, xe2 + f_tau2[1]*scale], 
                               [xe1,xe2]])
