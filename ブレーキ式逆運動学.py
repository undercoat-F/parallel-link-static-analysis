...
speed = 0.75  #描画する速度の方向の大きさ
direction_vector = [speed * math.cos(direction), speed * math.sin(direction)]
arrow_scale = 0.20 #手先速度の方向を表示する大きさを決める変数
arrow_dx1 = arrow_scale * direction_vector[0]
arrow_dx2 = arrow_scale * direction_vector[1]
# 手先速度と逆方向の角度を計算
direction_reverse = normalize_radian((direction + np.pi))  # 逆方向の角度を計算
direction_reverse_deg = np.degrees(direction_reverse)

reverse_dx1 = arrow_scale * speed * math.cos(direction_reverse)
reverse_dx2 = arrow_scale *  speed * math.sin(direction_reverse)
...
def draw_large_line(start_point, angle_rad):
    ...
    t = 100  #線の長さ
    x, y = x_start + t * direction_x, y_start + t * direction_y
...
while plt.fignum_exists(fig.number):
    ...
    #トルクの境界条件に関する処理
        ...
        if theta_v1_rotate_small> theta_v > theta_v2_rotate_small:
            tau1 = -1 * tau1_base
            tau2 = 1 * tau2_base
            area_flag = "Red"

        elif theta_v == theta_v2_rotate_small:
            tau1 = -1 * tau1_base
            tau2 = 0
        ...
        elif theta_v == theta_v1_rotate_large:
            tau1 = 0
            tau2 = -1 * tau2_base
        ...
        elif theta_v == theta_v2_rotate_large:
            tau1 = 1*tau1_base
            tau2 = 0
        ...
        elif theta_v == theta_v1_rotate_small:
            tau1 = 0
            tau2 = 1*tau2_base
        ...
        theta_v1_x1positions = [mouse_x1 + (line_length * np.cos(theta_v1_small)), mouse_x1 + (line_length * np.cos(theta_v1_large))]
        theta_v1_x2positions = [mouse_x2 + (line_length * np.sin(theta_v1_small)), mouse_x2 +(line_length * np.sin(theta_v1_large)) ]
        theta_v2_x1positions = [mouse_x1 + (line_length * np.cos(theta_v2_small)), mouse_x1 + (line_length * np.cos(theta_v2_large))]
        theta_v2_x2positions = [mouse_x2 + (line_length * np.sin(theta_v2_small)), mouse_x2 + (line_length * np.sin(theta_v2_large)) ]

        rect_size = 0.15
        theta_v1_x1rectpositions = [mouse_x1 + (rect_size * np.cos(theta_v1_small)), mouse_x1 + (rect_size * np.cos(theta_v1_large))]
        theta_v1_x2rectpositions = [mouse_x2 + (rect_size * np.sin(theta_v1_small)), mouse_x2 +(rect_size * np.sin(theta_v1_large)) ]
        theta_v2_x1rectpositions = [mouse_x1 + (rect_size * np.cos(theta_v2_small)), mouse_x1 + (rect_size * np.cos(theta_v2_large))]
        theta_v2_x2rectpositions = [mouse_x2 + (rect_size * np.sin(theta_v2_small)), mouse_x2 + (rect_size * np.sin(theta_v2_large)) ]
        #print(theta_v1_x1positions,theta_v1_x2positions)
        
        vertices =[
            (theta_v1_x1rectpositions[0] , theta_v1_x2rectpositions[0]),#左上
            (theta_v2_x1rectpositions[0] , theta_v2_x2rectpositions[0]),#右上
            (theta_v2_x1rectpositions[1] , theta_v2_x2rectpositions[1]),#左下
            (theta_v1_x1rectpositions[1] , theta_v1_x2rectpositions[1]) #右下
        ]
        
        triangles = [
            ([mouse_x1, vertices[0][0], vertices[1][0]], 
             [mouse_x2, vertices[0][1], vertices[1][1]], 'blue'),  # 上三角形
            ([mouse_x1, vertices[1][0], vertices[3][0]], 
             [mouse_x2, vertices[1][1], vertices[3][1]], 'blue'),  # 右三角形
            ([mouse_x1, vertices[3][0], vertices[2][0]], 
             [mouse_x2, vertices[3][1], vertices[2][1]], 'blue'),  # 下三角形
        ...
        ax.plot(theta_v1_x1positions, theta_v1_x2positions, 'c-', lw=1)#theta_v1を表す線
        ax.plot(theta_v2_x1positions, theta_v2_x2positions, 'm-', lw=1)#theta_v2を表す線
        ...
        reverse_vector = []
        '''
        if f_tau1_theta == direction_reverse:
            reverse_vector = f_tau1
        elif f_tau2_theta == direction_reverse:
            reverse_vector = f_tau2

        elif F_total_theta == direction_reverse:
        ...
        if abs(f_tau1_theta - direction_reverse) <= tolerance:
            ...
            if reverse_intersection is None:
                reverse_intersection = line_intersection((f1_tau1+mouse_x1, f2_tau1+mouse_x2),(F_total[0]+mouse_x1, F_total[1]+mouse_x2),(mouse_x1, mouse_x2),large_reverse_vector)
            if reverse_intersection is None:
                reverse_intersection = line_intersection((f1_tau2+mouse_x1, f2_tau2+mouse_x2),(F_total[0]+mouse_x1, F_total[1]+mouse_x2),(mouse_x1, mouse_x2),large_reverse_vector)
        ...
        ax.quiver(mouse_x1, mouse_x2, f_tau1[0] * scale_factor, f_tau1[1] * scale_factor, 
                angles='xy', scale_units='xy', scale=1, color='green', label=f"τ1 Force : {f_tau1}")

        # τ2 Force
        ax.quiver(mouse_x1, mouse_x2, f_tau2[0] * scale_factor, f_tau2[1] * scale_factor, 
                angles='xy', scale_units='xy', scale=1, color='orange', label=f"τ2 Force : {f_tau2}")
        ...
        if reverse_flag:
            ax.quiver(mouse_x1, mouse_x2, reverse_vector[0] * scale_factor, reverse_vector[1] * scale_factor, 
                    angles='xy', scale_units='xy', scale=1, color='purple', label=f"reverse_force : {reverse_vector}")
        else:
